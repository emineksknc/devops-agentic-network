...
class GitHubAgent(BaseAgent):
    ...
    async def fetch_commits(self, repo_owner: str, repo_name: str, count: int = 5) -> List[Dict[str, Any]]:
        """
        GitHub REST API'sine canlı token ile asenkron HTTP isteği atar.
        """
        ...
        # .env dosyasından gelen gerçek token'ı ekliyoruz
        ...
        return [] # Canlı bağlantıda hata varsa boş liste dönüyoruz ki akış güvenle dursun

    def extract_jira_ids(self, commit_messages: List[str]) -> List[str]:
        # Senin Jira board'undaki 'SCRUM' anahtarını yakalayacak esnek regex patenti
